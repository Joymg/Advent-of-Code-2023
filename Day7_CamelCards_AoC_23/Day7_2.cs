    ...
    internal class Day7_2
    {
        ...
        private static int CalculateTotalWinnings(string[] inputs)
        {
            ...
            for (int i = 0; i < hands.Count; i++)
            {
                ...
                //Console.WriteLine($"{hands[i].cards} {hands[i].bid}");
            }
            return sum;
        }
        ...
        public class Hand : IComparable<Hand>
        {
            ...
            private void CalculateHandType(string cards)
            {
                ...
                if (chars.ContainsKey('J') && chars.Count != 1 )
                {
                    int jValues = chars['J'];
                    chars.Remove('J');
                    int maxValue = chars.Values.Max();
                    List<Card> tmpCard = new();
                    List<KeyValuePair<char, int>> list = chars.Where(pair => pair.Value == maxValue).ToList();
                    if (list.Count > 1)
                    {
                        for (int i = 0; i < list.Count; i++)
                        {
                            tmpCard.Add(new Card(list[i].Key));
                        }
                        tmpCard.Sort();

                        chars[tmpCard[^1].card] += jValues;

                    }
                    else
                    {
                        chars[list[0].Key] += jValues;
                    }
        ...
        public class Card : IComparable<Card>
        {
            ...
            public List<char> cardStrength = new List<char>() { 'A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J' };
